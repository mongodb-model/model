...
class Migrate extends require("../base") {
  ...
  createDefaultDirectory(defaultPath) {
    if (!existsSync(require('path').join(process.cwd(), defaultPath))) {
  ...
  schemaMigration(command, model = new Model) {
    ...
    model.on('createCollection', this.onCreateCollection)
    model.createCollection(this.collectionName(command), require(join(process.cwd(), './app/schemas/' + command.split('=')[1] + '.js'))).then(response => {
      if (response && response.s && response.s.namespace) {
        console.log(Green("Migrated: " + response.s.namespace))
  ...
  migrationMigration(command, model = new Model) {
    ...
    model.on('createCollection', this.onCreateCollection)
    model.createCollection(this.collectionName(command), require(join(process.cwd(), './database/migrations/' + command.split('=')[1] + '.js'))).then(response => {
      if (response && response.s && response.s.namespace) {
        console.log(Green("Migrated: " + response.s.namespace))
  ...
  async migrateSchema(command, path = './app/schemas') {
    if (this.hasSchema(command)) {
      ...
      console.log(Red('No schema exists for this schema migration'))
    }
  }
  async migrateMigration(command, path = './database/migrations') {
    if (this.hasMigration(command)) {
      ...
      console.log(Red('No schema exists for this database migration'))
    }
  }
